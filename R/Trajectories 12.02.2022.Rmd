...
ols_estim_process <- ggplot()+
  ...
  annotate(geom="text", x=1.3,    y=lab_y, label="Final diversity", lineheight = .7,   col="grey50", size=lab_s, hjust = 1, alpha = 1) +
  geom_segment(aes(y=lab_y+.01,    x=1.3, yend = point_est_m_last_div_var1$ci_025-.01, xend = 1.3),size=0.3, col = "grey50", alpha=0.66, linetype=3) +
  
  annotate(geom="text", x=1.1,    y=lab_y, label="Final inclusion", lineheight = .7,   col="grey50", size=lab_s, hjust = 1, alpha = 1) +
  geom_segment(aes(y=lab_y+.01,    x=1.1, yend = point_est_m_last_inc_var1$ci_025-.01, xend = 1.1),size=0.3, col = "grey50", alpha=0.66, linetype=3) +
  
  annotate(geom="text", x=0.9,    y=lab_y, label="Trend of diversity", lineheight = .7,   col="grey50", size=lab_s, hjust = 1, alpha = 1) +
  geom_segment(aes(y=lab_y+.01,    x=0.9, yend = point_est_m_trend_div_var1$ci_025-.01, xend = 0.9),size=0.3, col = "grey50", alpha=0.66, linetype=3) +
  
  annotate(geom="text", x=0.7,    y=lab_y, label="Trend of inclusion", lineheight = .7,   col="grey50", size=lab_s, hjust = 1, alpha = 1) +
  geom_segment(aes(y=lab_y+.01,    x=0.7, yend = point_est_m_trend_inc_var1$ci_025-.01, xend = 0.7),size=0.3, col = "grey50", alpha=0.66, linetype=3) +
...
examples_1x2 <- cowplot::plot_grid(
                                     example_inc_to_div
                                     +ggtitle("a")
                                     #+scale_x_continuous(breaks=(1:3), expand = c(0.05,0.05), labels=c("Diversity \u00D7\nInclusion", "Inclusion", "Diversity"))
                                     +ylab("Z-score")
                                     +annotate(geom="text", x=1, y=1.5, label=expression(paste(italic("max("~T[I %->% D] ~ ")") )), hjust=0, color="black",size = 3.5)
                                     +theme(
                                    ...
                                    example_div_to_inc
                                     ...
                                     #+scale_x_continuous(breaks=(1:3), expand = c(0.05,0.05), labels=c("Diversity \u00D7\nInclusion", "Inclusion", "Diversity"))
                                     +ylab("Z-score")
                                     +annotate(geom="text", x=1, y=1.5, label=expression(paste(italic("max("~T[D %->% I] ~ ")") )), hjust=0, color="black",size = 3.5)
                                     +theme(
                                            plot.title =element_text(size=11),
                                            plot.margin = unit(c(m_top,m_right,m_bottom,m_left), "mm"),
                                            axis.title.x = element_text(size = 11, colour = "black"),
...
te_3_combined <- cowplot::plot_grid(
                                    examples_1x2
                                    
                                     +theme(#asp=1,
                                            plot.title =element_text(size=11, face='bold'),
                                            plot.margin = unit(c(m_top,m_right,m_bottom+2,m_left), "mm"),
                                            axis.text = element_blank()
                                            ),
                                    
                                     ols_estim_process
                                    +ggtitle("c")
                                    +ylab("SD change in dependent variable")
                                    +theme( aspect.ratio=1.3,
                                            #plot.title =element_text(size=11, face='bold'),
                                            plot.margin = unit(c(m_top+1.7,m_right,m_bottom+4,m_left), "mm"),
                                            axis.title.x = element_text(size = 11, colour = "black", face='plain'),
                                            #axis.text = element_blank()
                                            ),
...
```{r}
ggsave("process_3_panel.pdf", te_3_combined, width = 5, height = 4)
```

Save environment
```{r}
save.image(file='Trajectories.RData')
```
